...
static comm_request_t recv_requests[32768];
static comm_request_t send_requests[32768];
static comm_request_t send_stream_requests[32768];
static comm_request_t ready_requests[32768];

static comm_request_t timer_recv_requests[32768];
static comm_request_t timer_send_requests[32768];
...
int comm_regions_setup(int numReq, int type)
{
    ...
    {
        for(int index=0; index < numReq; index++)
            timer_recv_region[index] = (comm_reg_t*)calloc(1, 1*sizeof(comm_reg_t));
    }

    if(type == TIMER_SEND_REGION)
    {
        for(int index=0; index < numReq; index++)
            timer_send_region[index] = (comm_reg_t*)calloc(1, 1*sizeof(comm_reg_t));
    }
