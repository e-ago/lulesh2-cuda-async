...
int memReg=0;
  ...
  //comm_register_step(domain.commDataSend, SEND_REQUEST);
   ...
   if (planeMin | planeMax) {
      ...
      if (planeMin) {
        ...
        comm_global_isend_stream(destAddr, xferFields*sendCount, baseType,
                   ...
                   MPI_COMM_WORLD, &domain.sendRequest[pmsg], stream, index) ;
        ++pmsg;
      }
      memReg++;
      if (planeMax && doSend) {
...
int index = /* memReg;*/pmsg+emsg;
         ...
         comm_global_isend_stream(destAddr, xferFields*dz, baseType, toRank, msgType,
                   MPI_COMM_WORLD, &domain.sendRequest[pmsg+emsg], stream, index) ;
         ++emsg ;
      }
memReg++;
      if (rowMin && planeMin) {
...
int index = /* memReg;*/pmsg+emsg;
         ...
         comm_global_isend_stream(destAddr, xferFields*dx, baseType, toRank, msgType,
                   MPI_COMM_WORLD, &domain.sendRequest[pmsg+emsg], stream, index) ;
         ++emsg ;
      }
memReg++;
...
int index = /* memReg;*/pmsg+emsg;
         ...
         comm_global_isend_stream(destAddr, xferFields*dy, baseType, toRank, msgType,
                   MPI_COMM_WORLD, &domain.sendRequest[pmsg+emsg], stream, index) ;
         ++emsg ;
      }
memReg++;
...
int index = /* memReg;*/pmsg+emsg+cmsg;
         ...
         comm_global_isend_stream(comBuf, xferFields, baseType, toRank, msgType,
                   MPI_COMM_WORLD, &domain.sendRequest[pmsg+emsg+cmsg], stream, index) ;
         ++cmsg ;
      }
      memReg++;
...
//cudaDeviceSynchronize();
    ...
    {
      if(countReqs[i] != -1)
        comm_global_wait_stream(&domain.sendRequest[countReqs[i]], &status[countReqs[i]], SEND_REQUEST, stream, countReqs[i]);
...
//  comm_global_wait_all_stream(domain.sendRequest, status, 26, SEND_REQUEST, stream);
}
...
void CommSBNGpu(Domain& domain, int xferFields, Domain_member *fieldData, cudaStream_t *streams) {
   ...
   if (rowMin & colMin & planeMin) {
      ...
      Real_t *comBuf = &domain.commDataRecv[pmsg * maxPlaneComm +
                                             emsg * maxEdgeComm +
                                      cmsg * CACHE_COHERENCE_PAD_REAL] ;
      comm_global_wait_stream(&domain.recvRequest[pmsg+emsg+cmsg], &status, RECV_REQUEST, stream, pmsg+emsg) ;
      for (Index_t fi=0; fi<xferFields; ++fi) {
...
void CommSyncPosVelGpu(Domain& domain, cudaStream_t *streams) {
   ...
   PUSH_RANGE("RANK CommSyncPosVelGpu", 1);
   MPI_Comm_rank(MPI_COMM_WORLD, &myRank) ;
   POP_RANGE;

   if(myRank == 0 && print == 1)
    printf("***** CommSyncPosVelGpu *****\n");
