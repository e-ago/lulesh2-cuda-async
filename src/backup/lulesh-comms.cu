...
void CommRecv(Domain& domain, int msgType, Index_t xferFields,
   ...
   if (planeMin && doRecv) {
      ...
      int index = memReg; //pmsg
      comm_global_irecv(&domain.commDataRecv[pmsg * maxPlaneComm],
                recvCount, baseType, fromRank, msgType,
                MPI_COMM_WORLD, &domain.recvRequest[pmsg], index, domain.streams[2]);
   ...
   if (!planeOnly) {
      ...
      if (rowMin && colMin && doRecv) {
        ...
        int index = memReg; //pmsg+emsgt index = memReg; //pmsg+emsg
          comm_global_irecv(&domain.commDataRecv[pmsg * maxPlaneComm +
                                         emsg * maxEdgeComm],
                   dz * xferFields, baseType, fromRank, msgType,
                   MPI_COMM_WORLD, &domain.recvRequest[pmsg+emsg], index, domain.streams[2]) ;
         ++emsg ;
      }
memReg++;
      if (rowMin && planeMin && doRecv) {
        int fromRank = myRank - domain.tp()*domain.tp() - domain.tp() ;
        int index = memReg; //pmsg+emsg
          comm_global_irecv(&domain.commDataRecv[pmsg * maxPlaneComm +
                                         emsg * maxEdgeComm],
...
int index = memReg; //pmsg+emsg+cmsg
         comm_global_irecv(&domain.commDataRecv[pmsg * maxPlaneComm +
                                         emsg * maxEdgeComm +
                                         cmsg * CACHE_COHERENCE_PAD_REAL],
                   xferFields, baseType, fromRank, msgType,
                   MPI_COMM_WORLD, &domain.recvRequest[pmsg+emsg+cmsg], index, domain.streams[2]) ;
         ++cmsg ;
      }
      memReg++;
  ...
  cudaDeviceSynchronize();
   ...
   if (!planeOnly) {
      if (rowMin && colMin) {
        ...
        countReqs[indexReqs]=pmsg+emsg;
        indexReqs++;

         comm_global_isend(destAddr, xferFields*dz, baseType, toRank, msgType,
                   MPI_COMM_WORLD, &domain.sendRequest[pmsg+emsg], pmsg+emsg) ;
         ++emsg ;
      }
      ...
      countReqs[indexReqs]=pmsg+emsg;
        indexReqs++;
         ...
         comm_global_isend(destAddr, xferFields*dx, baseType, toRank, msgType,
                   MPI_COMM_WORLD, &domain.sendRequest[pmsg+emsg], pmsg+emsg) ;
         ++emsg ;
      }
      ...
      if (colMin && planeMin) {
        ...
        indexReqs++;
         ...
         comm_global_isend(destAddr, xferFields*dy, baseType, toRank, msgType,
                   MPI_COMM_WORLD, &domain.sendRequest[pmsg+emsg], pmsg+emsg) ;
         ++emsg ;
      }
      ...
      if (rowMin && colMin && planeMin) {
        ...
        indexReqs++;
         ...
         comm_global_isend(comBuf, xferFields, baseType, toRank, msgType,
                   MPI_COMM_WORLD, &domain.sendRequest[pmsg+emsg+cmsg], pmsg+emsg+cmsg) ;
         ++cmsg ;
      }
      if (rowMin && colMin && planeMax && doSend) {
        ...
        countReqs[indexReqs]=pmsg+emsg+cmsg;
        indexReqs++;
    ...
    {
      if(countReqs[i] != -1)
        comm_global_wait(&(domain.sendRequest[countReqs[i]]), &status[countReqs[i]], SEND_REQUEST, countReqs[i]);
    }

  //   comm_global_wait_all(domain.sendRequest, status, 26, SEND_REQUEST);
}
...
void CommSBN(Domain& domain, int xferFields, Domain_member *fieldData) {
   ...
   if (rowMin & colMin) {
      srcAddr = &domain.commDataRecv[pmsg * maxPlaneComm +
                                       emsg * maxEdgeComm] ;
      comm_global_wait(&domain.recvRequest[pmsg+emsg], &status, RECV_REQUEST, pmsg+emsg) ;
      for (Index_t fi=0 ; fi<xferFields; ++fi) {
         Domain_member dest = fieldData[fi] ;
         for (Index_t i=0; i<dz; ++i) {
